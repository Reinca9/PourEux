<?php 
include('header2.php')
?>

<div id="leCollectifArticleFlex">
    <article id="leCollectifArticle">
        <h1>
            Qui sommes nous?
        </h1>
        <hr id="leCollectifHr">
        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Nihil aspernatur voluptatem maiores tempore quis
            iste.
            Nemo ullam iste quos eius, voluptatem magni itaque possimus ipsam nihil aliquam, illum, nesciunt harum eaque
            a
            minus debitis reiciendis. Similique sequi quisquam vel quae!</p>
        <button id="jeMinscrisLeCollectifButton"><a id="jeMinscrisLeCollectif" href="index.php?page=signin">Je
                m'inscris</a></button>
    </article>
    <article>
        <img id="SecondArticleImg" src=".\assets\img\received_527396665639558.jpeg" alt="">
    </article>
</div>
<div id="TableauImgs">
    <div class="divFlexTableauImgs">
        <img class="TableauImgsFormat" src="./assets/img/chienPourEux.png" alt="">
        <img class="TableauImgsFormat" src=".\assets\img\Panier repas.jpg" alt="">
        <img class="TableauImgsFormat" src="./assets\img\280727706_1341566139673383_3589000400067496603_n.jpg" alt="">
    </div>
    <div class="divFlexTableauImgs">
        <img class="TableauImgsFormat" src="./assets\img\cuisine.jpg" alt="">
        <img class="TableauImgsFormat" id="logoPEuxTableauImgs" src="./assets\img\PourEuxNancySVG.svg" alt="">

        <img class="TableauImgsFormat" src="./assets\img\IMG_20211212_130328.jpg" alt="">
    </div>
    <div class="divFlexTableauImgs">
        <div class="TableauImgsFormat logoTableauImgsFormat" id="marginBottomImgs"><i id="instalogo"
                class="fa-brands fa-instagram" href="https://www.instagram.com/poureuxnancy/?hl=fr"></i>
        </div>
        <img class="TableauImgsFormat" src="./assets\img\241716767_1477515579286161_2201602846919997002_n.jpg" alt="">
        <div class="TableauImgsFormat logoTableauImgsFormat"><i id="fblogo" class="fa-brands fa-facebook"
                href="https://www.facebook.com/groups/832655407231327"></i></div>
    </div>
</div>
<article id="fondCouleurArticle">
    <div id="divChartePEux">
        <img id="chartePourEux" src="./assets\img\charte.png" alt="">
    </div>
    <div id="articlesChartePEux">
        <h1>Nos engagements</h1>
        <hr id="hrChartePEux">
        <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla beatae non deleniti ut illum ipsum
            dignissimos ad, eos, doloremque officiis, minima fugit. Velit, sed eius! Incidunt ducimus officiis placeat
            illo.
        </p>
        <a id="jeMinscrisLeCollectif2">Je m'inscris</a>
    </div>
</article>